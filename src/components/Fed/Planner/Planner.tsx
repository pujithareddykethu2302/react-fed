...
import { getGoalsData } from "../../service/dataSerice";
...
const Planner = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMonthlyGoals(loadGoals("monthlyGoals", data.monthlyGoals));
        setWeeklyGoals(loadGoals("weeklyGoals", data.weeklyGoals));
        setDailyGoals(loadGoals("dailyGoals", data.dailyGoals));
      } catch (error) {
  ...
  const handleAddGoal = (type: "monthly" | "weekly" | "daily") => {
    if (type === "monthly" && monthlyText.trim()) {
      const updated = [
        ...monthlyGoals,
        { text: monthlyText, completed: false },
      ];
  ...
  const toggleCompleted = (
    ...
    if (type === "monthly")
      setMonthlyGoals(update(monthlyGoals, "monthlyGoals"));
    else if (type === "weekly")
      setWeeklyGoals(update(weeklyGoals, "weeklyGoals"));
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-10">
      ...
      <p className="text-gray-600 mb-8">
        Plan your monthly, weekly, and daily goals to stay consistent and
        organized.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-md">
          <h2 className="text-xl font-semibold text-[#563A9C] mb-1">
            Monthly Goals
          </h2>
          ...
          <ul className="space-y-2">
            {monthlyGoals.map((goal, i) => (
              <li
                key={i}
                className="flex items-center gap-2 bg-gray-50 border border-gray-200 p-3 rounded-lg text-gray-700"
              >
                <input
                  type="checkbox"
                  checked={goal.completed}
                  onChange={() => toggleCompleted("monthly", i)}
                />
                <span
            ...
            {weeklyGoals.map((goal, i) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={goal.completed}
                  onChange={() => toggleCompleted("weekly", i)}
                />
                <span
                  className={goal.completed ? "line-through text-gray-400" : ""}
                >
            ...
            {dailyGoals.map((goal, i) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={goal.completed}
                  onChange={() => toggleCompleted("daily", i)}
                />
                <span
                  className={goal.completed ? "line-through text-gray-400" : ""}
                >
      ...
      <div className="mt-16 bg-white rounded-xl shadow-md flex flex-col md:flex-row items-center p-8 gap-8">
        <div className="flex-1">
          ...
          <p className="text-gray-600">
            Keep planning consistently and watch your goals turn into
            achievements.
          </p>
        </div>
        <div className="flex-1">
