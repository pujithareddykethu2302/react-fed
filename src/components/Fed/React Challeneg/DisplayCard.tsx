            ...
            {loading ? (
                ...
                <div className="">
                    ...
                    <div className="flex justify-center items-center">
                        <button className="bg-gray-300 w-[2.5rem] h-[2.5rem]  mr-[5px]  border disabled:opacity-30 border-gray-300 cursor-pointer disabled:cursor-default" type="button" 
                        disabled={CurrentPage === 1}
                        onClick={()=>{setCurrentPage((p)=> p-1)}}>˂</button>
                        {PageNumbers.map((i) => {
                            return (
                                <>
                                    <button className= {` w-[2.5rem] h-[2.5rem] border mr-[5px] cursor-pointer ${CurrentPage == i ? "bg-[#563A9C] text-white border" : "border border-gray-300"}`} type="button" onClick={()=>setCurrentPage(i)}>{i}</button>
                                </>
                            )
                        })}
                        <button className="bg-gray-300 w-[2.5rem] h-[2.5rem] border border-gray-300 cursor-pointer disabled:cursor-default disabled:opacity-10" 
                        onClick={()=>{setCurrentPage((p)=> p+1)}}
                         disabled={CurrentPage === TotalPages}type="button">˃</button>
                    </div>
                </div>
            ) : (
